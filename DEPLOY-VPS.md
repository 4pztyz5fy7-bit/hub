# КВАНТ — перенос сайта на Timeweb Cloud (VPS + домен + почта)
...
**Схема:** всё в одном месте — **Timeweb Cloud**: VPS (сервер), домен `kvantm.tech` (уже перенесён с reg.ru) и почта `@kvantm.tech`.

**Порядок действий:**
1. Купить VPS в Timeweb Cloud
2. Подключиться к серверу с компьютера
3. Настроить сервер (по одной команде)
4. Настроить DNS домена `kvantm.tech` в Timeweb Cloud (→ IP вашего VPS)
...
9. Настроить админку и почту Timeweb Cloud (SMTP)
...
## Шаг 4. Настроить DNS домена `kvantm.tech` в Timeweb Cloud

Домен уже перенесён с reg.ru в **Timeweb Cloud**, поэтому все настройки делаем в одной панели — там же, где и VPS. Нужно, чтобы `kvantm.tech` указывал на IP вашего VPS.

1. Войти в **Timeweb Cloud** → левое меню → **Домены**.
2. В списке найти `kvantm.tech` → нажать на него.
3. Открыть вкладку **DNS-записи** (иногда «Управление DNS» / «DNS-зона»).
4. Убедитесь, что для домена в разделе **NS-серверы** указаны **NS Timeweb Cloud** (обычно `ns1.timeweb.cloud`, `ns2.timeweb.cloud`, `ns3.timeweb.cloud`, `ns4.timeweb.cloud` — точные значения показаны в панели). Если стоят старые NS от reg.ru — переключите на Timeweb Cloud, иначе записи ниже не сработают.
5. Если увидите старые записи типа **A** для `@` или `www`, ведущие не на ваш VPS — **удалить их**.
6. Нажать **Добавить запись**:
   ```
   Тип:      A
   Имя:      @        (или оставить пустым — означает корень домена)
   Значение: IP-адрес вашего VPS из Шага 1 (например 194.87.123.45)
   TTL:      600
   ```
   Сохранить.
7. Ещё раз **Добавить запись**:
   ```
   Тип:      A
   Имя:      www
   Значение: тот же IP VPS
   TTL:      600
   ```
   Сохранить.
...
> ⚠️ Не трогайте MX-записи и записи `mail`, `_dmarc`, `_domainkey`, `spf` — они нужны для почты Timeweb Cloud (Шаг 9.2). Их Timeweb создал автоматически при подключении почты.
...
Если в ответах видите **IP вашего VPS** — готово, идите дальше. Если старый IP или ошибка — ждите ещё.
...
3. `Note`: `vps-hub`, `Expiration`: `90 days`, галочка **`repo`**.
...
```bash
git clone https://ВАШЛОГИН:ВАШТОКЕН@github.com/ВАШЛОГИН/hub.git
```
...
```bash
pm2 start "bun run start" --name hub --cwd /var/www/hub
pm2 save
pm2 startup systemd
```
...
```bash
nano /etc/systemd/system/hub.service
```
...
```bash
docker build -t hub .
docker run -d --name hub -p 3000:3000 --env-file /var/www/hub/.env --restart unless-stopped hub
```
...
```bash
cd /var/www/hub
nohup bun run start > /var/log/hub.log 2>&1 &
```
...
- **PM2:** `pm2 restart hub`
- **systemd:** `systemctl restart hub`
- **Docker:** `docker restart hub`
...
```bash
echo "alias update='cd /var/www/hub && git pull && bun install && bun run build && pm2 restart hub'" >> ~/.bashrc
```

Если используете **systemd**:

```bash
echo "alias update='cd /var/www/hub && git pull && bun install && bun run build && systemctl restart hub'" >> ~/.bashrc
```

Если используете **Docker**:

```bash
echo "alias update='cd /var/www/hub && git pull && bun install && bun run build && docker restart hub'" >> ~/.bashrc
```
...
|---|---|---|---|
| Статус сайта | `pm2 status` | `systemctl status hub` | `docker ps` |
| Свежие ошибки | `pm2 logs hub` (выход — Ctrl+C) | `journalctl -u hub -f` (выход — Ctrl+C) | `docker logs -f hub` |
| Перезапуск | `pm2 restart hub` | `systemctl restart hub` | `docker restart hub` |
| Остановить | `pm2 stop hub` | `systemctl stop hub` | `docker stop hub` |
...
|---|---|
| Сайт не открывается вообще | Проверьте статус по выбранному способу: `pm2 status`, `systemctl status hub` или `docker ps`. Посмотрите логи. |
| Открывается только по IP, но не по домену | DNS ещё не обновился — подождать; проверить `ping kvantm.tech` |
| `502 Bad Gateway` | Перезапустить процесс (`pm2 restart hub` / `systemctl restart hub` / `docker restart hub`) и посмотреть логи |
